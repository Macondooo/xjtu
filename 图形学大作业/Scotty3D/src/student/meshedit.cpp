...
std::optional<Halfedge_Mesh::VertexRef> Halfedge_Mesh::collapse_edge(Halfedge_Mesh::EdgeRef e) {
    ...
    new_v->pos = e->center();
    EdgeRef new_e1 = new_edge();
    EdgeRef new_e2 = new_edge();
    FaceRef f1 = e->halfedge()->face();
    FaceRef f2 = e->halfedge()->twin()->face();
    VertexRef a = e->halfedge()->vertex();
    VertexRef b = e->halfedge()->twin()->vertex();

    std::vector<HalfedgeRef> around_f1;
    around_f1.push_back(e->halfedge());
    around_f1.push_back(e->halfedge()->next());
    around_f1.push_back(e->halfedge()->next()->next());
    std::vector<HalfedgeRef> around_f2;
    around_f2.push_back(e->halfedge()->twin());
    around_f2.push_back(e->halfedge()->twin()->next());
    around_f2.push_back(e->halfedge()->twin()->next()->next());

    // set halfedge
    HalfedgeRef h = a->halfedge();
    do {
        h->_vertex = new_v;
        h = h->twin()->next();
    } while(h != a->halfedge());
    ...
    around_f1[1]->twin()->_edge = new_e1;
    around_f1[1]->twin()->_twin = around_f1[2]->twin();
    around_f1[2]->twin()->_edge = new_e1;
    around_f1[2]->twin()->_twin = around_f1[1]->twin();

    around_f2[1]->twin()->_edge = new_e2;
    around_f2[1]->twin()->_twin = around_f2[2]->twin();
    around_f2[2]->twin()->_edge = new_e2;
    around_f2[2]->twin()->_twin = around_f2[1]->twin();

    // set vertex
    new_v->_halfedge = around_f1[2]->twin();
    around_f1[2]->vertex()->_halfedge = around_f1[1]->twin();
    around_f2[2]->vertex()->_halfedge = around_f2[1]->twin();

    // set edge
    new_e1->_halfedge = around_f1[1]->twin();
    new_e2->_halfedge = around_f2[1]->twin();

    erase(e);
    erase(around_f1[1]->edge());
    erase(around_f1[2]->edge());
    erase(around_f2[1]->edge());
    erase(around_f2[2]->edge());
    for(HalfedgeRef& x : around_f1) erase(x);
    for(HalfedgeRef& x : around_f2) erase(x);
    erase(f1);
    erase(f2);
    erase(a);
    erase(b);
...
struct Edge_Record {
    ...
    Edge_Record(std::unordered_map<Halfedge_Mesh::VertexRef, Mat4>& vertex_quadrics,
        ...
        // -> Build the 3x3 linear system whose solution minimizes the quadric error associated with
        // these two endpoints.
        Mat4 tmp = vertex_quadrics[v1] + vertex_quadrics[v2];
        Mat4 A = Mat4::I;
        for(int i = 0; i <= 2; i++)     // col
            for(int j = 0; j <= 2; j++) // line
                A[i][j] = tmp[i][j];

        Vec3 b = Vec3(-tmp[3][0], -tmp[3][1], -tmp[3][2]);
        // -> Use this system to solve for the optimal position, and store it in
        // Edge_Record::optimal.
        optimal = A.inverse() * b; // default pad b with 1.0
        // -> Also store the cost associated with collapsing this edge in Edge_Record::cost.
        cost = dot(Vec4(optimal, 1.0f), (tmp * Vec4(optimal, 1.0f)));
    }
...
bool Halfedge_Mesh::simplify() {
    ...
    for(FaceRef f = faces_begin(); f != faces_end(); f++) {
        Vec3 p1 = f->halfedge()->vertex()->pos;
        Vec3 p2 = f->halfedge()->next()->vertex()->pos;
        Vec3 p3 = f->halfedge()->next()->next()->vertex()->pos;

        Vec3 n = cross(p2 - p1, p3 - p1);
        float d = -dot(n, p1);
        Vec4 tmp = Vec4(n, d);
        Mat4 K = outer(tmp, tmp);
        face_quadrics.insert(std::make_pair(f, K));
    }
    ...
    for(VertexRef v = vertices_begin(); v != vertices_end(); v++) {
        ...
        do {
            FaceRef neighbor = h->face();
            K += face_quadrics[neighbor];
            h = h->twin()->next();
        } while(h != v->halfedge());

        vertex_quadrics.insert(std::make_pair(v, K));
    }
    ...
    for(EdgeRef e = edges_begin(); e != edges_end(); e++) {
        ...
        edge_records.insert(std::make_pair(e, record));
    ...
    //    top of the queue.
    int origin_f = int(faces.size());
    int count_f = 0;
    while(count_f < origin_f / 4 * 3) {

        if(edge_queue.size() == 0) {
            info("can't simplify anymore");
            return false;
            // break;
        }
        ...
        VertexRef a = e->halfedge()->vertex();
        VertexRef b = e->halfedge()->twin()->vertex();
        std::list<EdgeRef> a_nei_e;
        std::list<EdgeRef> b_nei_e;
        std::unordered_set<VertexRef> a_nei_v;

        // check if it can be collapsed
        // get a's neighors
        HalfedgeRef ha = a->halfedge();
        do {
            a_nei_v.insert(ha->twin()->vertex());
            a_nei_e.push_back(ha->edge());
            ha = ha->twin()->next();
        } while(ha != a->halfedge());
        // find how many duplicate neighbors
        int count = 0;
        HalfedgeRef hb = b->halfedge();
        do {
            VertexRef v = hb->twin()->vertex();
            b_nei_e.push_back(hb->edge());
            if(a_nei_v.find(v) != a_nei_v.end()) count++;
            hb = hb->twin()->next();
        } while(hb != b->halfedge());
        ...
        // save the K and erase the vertex
        Mat4 newv_K = vertex_quadrics[a] + vertex_quadrics[b];
        vertex_quadrics.erase(a);
        vertex_quadrics.erase(b);
        // erase the edge
        for(EdgeRef& x : a_nei_e) {
            edge_queue.remove(edge_records[x]);
            edge_records.erase(x);
        }
        for(EdgeRef& x : b_nei_e) {
            edge_queue.remove(edge_records[x]);
            edge_records.erase(x);
        }

        std::optional<VertexRef> new_v = collapse_edge_erase(e);
        new_v.value()->pos = record.optimal;
        vertex_quadrics[new_v.value()] = newv_K;

        // push every edge of new_v into queue
        HalfedgeRef h = new_v.value()->halfedge();
        do {
            EdgeRef et = h->edge();
            Edge_Record rec = Edge_Record(vertex_quadrics, et);
            edge_records.insert(std::make_pair(et, rec));
            edge_queue.insert(rec);
            h = h->twin()->next();
        } while(h != new_v.value()->halfedge());
