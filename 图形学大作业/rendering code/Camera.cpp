...
glm::mat4 Camera::lookAt(const glm::vec4 &eye, const glm::vec4 &at, const glm::vec4 &up)
{
	...
	// glm::vec4 t =
	glm::mat4 c = glm::mat4(1.0f); // 这一行需要修改
	c[0] = u;
	c[1] = v;
	c[2] = n;
	c= glm::transpose(c);
						...
						const GLfloat zNear, const GLfloat zFar)
{
	...
	c[3][3] = 1.0f;
	c[0][3] = -(left + right) / (right - left);
	c[1][3] = -(bottom + top) / (top - bottom);
	c[2][3] = -(zNear + zFar) / (zFar - zNear);
	//c[2][3] = -(zNear + zFar) / (zNear - zFar);
	c = glm::transpose(c);
	return c;
}
							  ...
							  const GLfloat zNear, const GLfloat zFar)
{
	...
	c[2][3] = -2.0 * zNear * zFar / (zFar - zNear);
	c[3][2] = -1.0;
	c[3][3] = 0.0;
	c = glm::transpose(c);
	return c;
